...
//lead{
LLMがツールを使って複雑な処理をこなすようになった今、応答が返るまでの「待ち時間」はユーザー体験の一つの課題です。本章では、Chainlit の @<code>{cl.Step} を起点に、Markdown・TaskList・Plotly・非同期並行処理という4つのアプローチで待ち時間の不安を解消する方法をいくつか紹介します。
//}

== 本章で利用するアプリケーション

本章では @<fn>{support} のリサーチツールを例に、Chainlit の Step 機能を中心に説明します。このアプリケーションは、ユーザーがブラウザから質問を送ると、OpenAI の @<code>{web_search_preview} ツールを使って複数のトピックをウェブ検索し、結果を集約して回答を返すチャットボットです。
...
本章では、思考の過程をわかりやすく表示する工夫と、待ち時間を有効活用する工夫を紹介します。

過程を何も表示しない場合、@<img>{kotsubo/no_step} のようにローディングインジケーターだけが表示され、処理が本当に進んでいるのか判断できません。
...
本章のリサーチツールでは、次のような3段階の入れ子構造で Step を構成しています。

//emlist[ベースの実装（main 関数の抜粋）]{
@cl.on_message
...
変更内容	主なメリット
----------------
Step の output をテーブル形式に	ソース・信頼度が展開不要で一覧できる
サイドバーに進捗リストを追加	全体の何件中何件が完了したか一目でわかる
信頼度ヒートマップを追加	ソース品質の比較が色で直感的にできる
豆知識の並行生成	待ち時間を有効活用し初動表示を早める
//}

それぞれの修正は独立していて、組み合わせて使うことも単独で利用することも可能です。
Step の表示を工夫するだけでなく、待ち時間そのものを有意義なものに変える発想が、LLM アプリケーションの UX 改善には重要です。
