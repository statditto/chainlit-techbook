...
//emlist[app.py - PCM→WAV変換][python]{
def pcm_to_wav(
...
//emlist[app.py - Whisper APIによる文字起こし][python]{
@cl.on_audio_end
async def on_audio_end():
    """録音終了時に音声を文字起こしし、LLM に送信する。"""
    audio_buffer: bytes = cl.user_session.get("audio_buffer")
...
=== 会話レビュー機能
せっかくの英会話なので、単純に話すだけでなく、自分の英語のどこが良いのか、どこを改善すればいいのかフィードバックをもらえるような機能も実装してみましょう。
