...
チャットを再開するには、チャット履歴が保存されている必要があります。
...
典型的には、アシスタントが利用するLLMやそのハイパーパラメータを指定できます。
...
もしユーザーにメッセージ入力を要求したい状況では、 @<code>{cl.AskUserMessage} および @<code>{cl.AskMessageBase} を具象化したクラスが有用です。
