...
//footnote[color-tokens][スタイルガイド https://seahawk-tiger.github.io/chainlit-ui-appendix/pages/appendix1-color-and-font.html]
//footnote[design-color][デザイン案 https://seahawk-tiger.github.io/chainlit-ui-appendix/pages/appendix2-ui-design-proto.html]
//footnote[logo-file-url][ロゴのSVGファイル https://github.com/seahawk-tiger/chainlit-ui-playground/tree/base/sample_files]
...
まず、.chainlitディレクトリと同じ階層にpublicディレクトリを作り、faviconとロゴに使うファイルを配置します@<fn>{logo-file-url}。
...
その後、config.tomlを開き、配置したロゴファイルのパスを入力すれば完了です。
...
補足として、config.tomlには他にもログインページの背景画像等の設定項目があります。
...
その後、theme.jsonにあらかじめ設計したカラーや枠線のスタイルを記載します。この記法はShadcnのTheme記法に従います。下記に設定例を示しますが、詳細は章末のリンクを参照してください。
...
続いて、フォームを中央に配置する処理です。まず、フォームを含むrootを全面に広げ中央配置します。その後、フォームを持つ要素を中央寄せします。
//emlist[public/stylesheet.css][css]{
...
===== ロゴを入力欄の上部に表示する @<br>{}
ロゴを画面の中心かつ入力欄の上に配置するため、logo.svgを使った要素を探して、中央寄せします。
...
//image[seahawk-default-bubble-ml][標準の吹き出しデザイン][scale=0.6]
...
その前に、SVGファイルは、publicにlogosというディレクトリを作ってまとめて格納しておきます@<fn>{logo-file-url}。
