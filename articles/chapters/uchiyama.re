...
チャットUI付きのLLMアプリケーションをで簡単に作るためのフレームワーク、というイメージが強いChainlit。
...
本稿で取り扱うソースコードはサポートページ(https://github.com/y-uty/chainlit-observation)で公開していますので、ハンズオンに取り組みたい方はリポジトリを実行環境にcloneして uv sync で環境を構築してください。
...
//}
記録したメタデータは、トークン数の推移を追跡したコストを見積もり、異なるモデル間のパフォーマンスを比較、@<code>{finish_reason} でエラーやトークン上限到達の検出などに活用できます。

=== ハンズオン: LLM呼び出しの観察
実際の観察コードつきChainlitを動かして、LLMに何か質問して結果を確認してみましょう。
以下のコマンドでChainlitを起動し、ブラウザで http://localhost:8000 を開いてください。
//emlist[][bash]{
...
//image[uchiyama/ch2_response][質問への回答が表示された状態]{
//}
...
//image[uchiyama/ch2_jsondisp][JSON形式で描画されたメタデータ]{
//}
...
//image[uchiyama/ch301_nested_steps][ステップの階層. 各LLM呼び出しはさらにTool callの子ステップを持つ]{
//}

//image[uchiyama/ch302_llmcall01][LLM呼び出し1回目]{
//}

//image[uchiyama/ch303_getwether_tokyo][東京の天気情報を取得]{
//}

//image[uchiyama/ch304_getwether_osaka][大阪の天気情報を取得]{
//}

//image[uchiyama/ch305_llmcall02][LLM呼び出し2回目]{
//}

//image[uchiyama/ch306_add][加算]{
//}

//image[uchiyama/ch307_llmcall03][LLM呼び出し3回目(最終回答)]{
//}
...
そこで、以下の4つの要約スタイルをプロンプトとしてLLMに支持し、その要約結果、トークン数・レイテンシを比較してみます。
//emlist[][]{
シンプル: 最小限の指示（ベースライン）
詳細指示: 具体的な要件や制約を含む
...
テキストを入力すると、4つのスタイルそれぞれ対する要約結果と、パフォーマンス比較の表が表示されます。
//image[uchiyama/ch401_res_detail][要約結果の例(詳細指示)]{
//}

//image[uchiyama/ch402_res_compare][スタイルごとの比較]{
//}
